...
class _CabOrderDetailScreenState extends State<CabOrderDetailScreen> {
  ...
  void initState() {
    ...
    totalAmount = amountShow(
        amount: (double.parse(orderModel?.subTotal?.toString() ?? "0.0") -
                double.parse(orderModel?.discount?.toString() ?? "0.0") +
                taxAmount)
            .toString());
    ...
    // print("Datat ${double.parse(orderModel?.adminCommission?.toString() ?? "0.0")}");
        ...
        child: Column(
          children: [
            ...
            (orderModel!.paymentStatus == "UnPaid" ||
                    orderModel!.status == ORDER_STATUS_CANCELLED)
                ? SizedBox()
                : Card(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(),
          ListView.builder(
            itemCount: orderModel?.taxModel?.length ?? 0,
            shrinkWrap: true,
