...
import 'package:door_delights_driver/controllers/driver_create_controller.dart';
...
class DriverCreateScreen extends StatelessWidget {
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // ── Per‑section vehicle card (only for selected cab/rental) ─
                            if (controller.selectedSection.value != null &&
                                controller.sectionNeedsVehicle(
                                    controller.selectedSection.value!))
                              _buildVehicleCard(
                                controller,
                                controller.selectedSection.value!,
  ...
  // ── Helper: build vehicle card for the selected section ──────────────
  Widget _buildVehicleCard(
    DriverCreateController controller,
    ...
    final selectedRideType =
        controller.selectedRideTypePerSection[sid] ?? RxString('ride');
    final selectedCarMakes =
        controller.selectedCarMakesPerSection[sid] ?? Rx<CarMakes>(CarMakes());
    final carModels =
        controller.carModelListPerSection[sid] ?? <CarModel>[].obs;
    final selectedCarModel =
        controller.selectedCarModelPerSection[sid] ?? Rx<CarModel>(CarModel());
    final carPlate = controller.carPlatePerSection[sid] ??
    ...
    return _SectionVehicleCard(
      ...
      isCab: section.serviceType == 'cab-service',
      ...
      carPlate: carPlate,
      onBrandChanged: () => controller.getCarModelForSection(sid),
      isDark: isDark,
    );
  }
  ...
  void _onSave(DriverCreateController controller, bool isEdit) {
    ...
    // Vehicle validation for selected section (if it needs a vehicle)
    final section = controller.selectedSection.value!;
    if (controller.sectionNeedsVehicle(section)) {
      final sid = section.id ?? '';
...
// ── Per‑section vehicle card (unchanged) ──────────────────────────────────
