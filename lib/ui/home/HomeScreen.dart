...
class HomeScreenState extends State<HomeScreen>
  ...
  @override
  void initState() {
    orderModel = Get.arguments?['orderModel'] as OrderModel?;
  ...
  getCurrentOrder(String inProgressOrderId) async {
    ordersFuture = FireStoreUtils().getOrderByID(inProgressOrderId);
    ...
    _orderSubscription = ordersFuture.listen((event) {
      ...
      if (currentOrder!.status == ORDER_STATUS_COMPLETED) {
        ...
        Constant.userModel!.deliveryInProgressOrderIds
            ?.removeWhere((orderId) => orderId == currentOrder!.id);
    ...
    _timer = new Timer.periodic(
      ...
      (Timer timer) async {
        if (driverOrderAcceptRejectDuration == 0) {
          ...
          if (_driverModel.deliveryOrderRequests != null &&
              _driverModel.deliveryOrderRequests?.firstWhereOrNull(
                      (order) => order.id == orderModel!.id) !=
                  null) {
            await rejectOrder();
          }
        } else {
  ...
  getDriver() async {
    ...
    _driverSubscription = driverStream.listen((event) async {
      ...
      if (orderModel != null &&
          _driverModel!.deliveryInProgressOrderIds != null &&
          _driverModel!.deliveryInProgressOrderIds!.isNotEmpty) {
        inProgressOrderid = _driverModel!.deliveryInProgressOrderIds
            ?.firstWhereOrNull((orderId) => orderId == orderModel!.id);
      } else {
        if (_driverModel!.inProgressOrderID != null) {
          inProgressOrderid = _driverModel!.inProgressOrderID!;
        }
      }

      if (inProgressOrderid != null) {
        getCurrentOrder(inProgressOrderid);
      }
    });
  }
      ...
      return Scaffold(
        ...
        body: Column(
          children: [
            ...
            (orderModel != null &&
                    _driverModel != null &&
                    (_driverModel!.deliveryOrderRequests
                            ?.any((order) => order.id == orderModel!.id) ??
                        false))
                ? showDriverBottomSheet()
                : Container()
          ],
        ),
        floatingActionButton: (_driverModel!.deliveryOrderRequests != null &&
                    _driverModel!.deliveryOrderRequests!.isNotEmpty) ||
                (_driverModel!.deliveryInProgressOrderIds == null ||
                    _driverModel!.deliveryInProgressOrderIds!.isEmpty)
            ? null
  ...
  void _updateMarkers(
    ...
    setState(() {
      ...
      } else if (orderModel!.status == ORDER_STATUS_DRIVER_PENDING) {
        // New order request - going to vendor
        _markers['Vendor'] = Marker(
          markerId: const MarkerId('Vendor'),
          infoWindow: InfoWindow(title: orderModel!.vendor!.title),
          position: LatLng(
              orderModel!.vendor!.latitude!, orderModel!.vendor!.longitude!),
          icon: departureIcon!,
        );
      }
    });
  ...
  acceptOrder() async {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        _driverModel!.deliveryOrderRequests
            ?.removeWhere((order) => order.id == orderModel.id);
        _driverModel!.inProgressOrderID = orderModel.id;
        _driverModel!.deliveryInProgressOrderIds?.add(orderModel.id!);
  ...
  completeOrder() async {
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        _driverModel!.deliveryInProgressOrderIds
            ?.removeWhere((order) => order == orderModel!.id);
