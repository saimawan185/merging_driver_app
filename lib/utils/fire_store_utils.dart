...
import 'package:door_delights_driver/app/chat_screens/ChatVideoContainer.dart';
...
import 'package:door_delights_driver/models/conversation_model.dart';
...
import 'package:door_delights_driver/models/email_template_model.dart';
import 'package:door_delights_driver/models/inbox_model.dart';
import 'package:door_delights_driver/models/mail_setting.dart';
import 'package:door_delights_driver/models/notification_model.dart';
import 'package:door_delights_driver/models/on_boarding_model.dart';
...
import 'package:door_delights_driver/models/payment_model/cod_setting_model.dart';
import 'package:door_delights_driver/models/payment_model/flutter_wave_model.dart';
import 'package:door_delights_driver/models/payment_model/mercado_pago_model.dart';
import 'package:door_delights_driver/models/payment_model/mid_trans.dart';
...
import 'package:door_delights_driver/models/payment_model/pay_stack_model.dart';
import 'package:door_delights_driver/models/payment_model/paypal_model.dart';
import 'package:door_delights_driver/models/payment_model/paytm_model.dart';
import 'package:door_delights_driver/models/payment_model/razorpay_model.dart';
import 'package:door_delights_driver/models/payment_model/stripe_model.dart';
import 'package:door_delights_driver/models/payment_model/wallet_setting_model.dart';
import 'package:door_delights_driver/models/payment_model/xendit.dart';
import 'package:door_delights_driver/models/referral_model.dart';
...
import 'package:door_delights_driver/models/withdrawal_model.dart';
...
class FireStoreUtils {
  ...
  static Future<bool> updateUser(UserModel userModel) async {
    try {
      ...
      log("Failed to update user 2: $error");
      log('Error 2: ' + stack.toString());
      return false;
    }
  }
  ...
  static Future<bool?> setWalletTransaction(
    ...
    await fireStore
      ...
      log("Failed to update user 3: $error");
      isAdded = false;
    });
    return isAdded;
  }
  ...
  static Future<bool?> setOrder(OrderModel orderModel) async {
    ...
    await fireStore
      ...
      log("Failed to update user 4: $error");
      isAdded = false;
    });
    return isAdded;
  }
  ...
  static Future<bool?> setParcelOrder(ParcelOrderModel orderModel) async {
    ...
    await fireStore
      ...
      log("Failed to update user 5: $error");
      isAdded = false;
    });
    return isAdded;
  }
  ...
  static Future<void> updateWallateAmount(OrderModel orderModel) async {
    ...
    for (var element in orderModel.products!) {
      final double price =
          ((double.tryParse(element.discount_price.toString()) ?? 0) > 0)
              ? double.parse(element.discount_price.toString())
              : double.parse(element.price.toString());

      final double qty = double.parse(element.quantity.toString());
      final double extras = double.parse(element.extras_price.toString());
    ...
    if (orderModel.taxScope == "product") {
      for (var element in orderModel.products!) {
        final double price =
            (double.parse(element.discount_price.toString()) > 0)
                ? double.parse(element.discount_price.toString())
                : double.parse(element.price.toString());
    ...
    packagingCharge = orderModel.packagingChargeEnable == true
    ...
    if (isCOD) {
      ...
      driverAmount = -((subTotal - totalDiscount) +
          productTaxAmount +
          orderTaxAmount +
          packagingTaxAmount +
          platformTaxAmount +
  ...
  static Future<bool> withdrawWalletAmount(WithdrawalModel userModel) async {
    ...
    await fireStore
      ...
      log("Failed to update user 6: $error");
      isUpdate = false;
    });
    return isUpdate;
  }
    ...
    for (final did in driverIds) {
      ...
      allOrders.addAll(
        snapshot.docs.map((doc) =>
            CabOrderModel.fromJson(doc.data() as Map<String, dynamic>)),
      );
    }
    return allOrders;
  }
