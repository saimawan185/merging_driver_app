...
class RentalReviewController extends GetxController {
  ...
  Future<void> getReview() async {
    final existingRating =
        await FireStoreUtils.getReviewsbyID(order.value?.id ?? "");
    ...
    final user =
        await FireStoreUtils.getUserProfile(order.value?.authorID ?? '');
    ...
    if (user != null) {
      ...
      final int userReviewsSum =
          int.tryParse(user.reviewsSum?.toString() ?? "0") ?? 0;
  ...
  Future<void> submitReview() async {
    ...
    if (user != null) {
      user.reviewsCount = (futureCount.value + 1);
      user.reviewsSum = (futureSum.value + ratings.value.toInt());
    }
