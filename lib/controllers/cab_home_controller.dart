...
class CabHomeController extends GetxController {
  ...
  SectionModel? get _activeSectionModel {
    final sid = (currentOrder.value.sectionId?.isNotEmpty == true)
        ...
        : (driverModel.value.orderCabRequestData?.sectionId?.isNotEmpty == true)
            ...
            : driverModel.value.sectionId?.isNotEmpty == true
  ...
  Future<BitmapDescriptor> _getDriverIcon() async {
    ...
    if (iconUrl != null && iconUrl.isNotEmpty) {
      if (iconUrl != _cachedIconUrl || _cachedSectionIcon == null) {
        ...
        _cachedSectionIcon =
            await _bitmapDescriptorFromUrl(iconUrl, width: 120);
      }
  ...
  Widget _driverMarkerWidget() {
    ...
    if (iconUrl != null && iconUrl.isNotEmpty) {
      return CachedNetworkImage(
        ...
        fit: BoxFit.contain,
        placeholder: (_, __) =>
            Image.asset('assets/images/ic_cab.png', width: 45, height: 45),
  ...
  Future<void> rejectOrder() async {
    try {
      ...
      driverModel.value.orderCabRequestData = null;
      driverModel.value.inProgressOrderID = null;
      await FireStoreUtils.updateUser(driverModel.value);
    ...
    try {
      if (event.exists && event.data() != null) {
        ...
        if (driverModel.value.id != null) {
          ...
          final activeSectionId = (currentOrder.value.sectionId?.isNotEmpty ==
                  true)
              ? currentOrder.value.sectionId
              : (driverModel.value.orderCabRequestData?.sectionId?.isNotEmpty ==
                      true)
                  ? driverModel.value.orderCabRequestData!.sectionId
                  : driverModel.value.sectionId?.isNotEmpty == true
                      ? driverModel.value.sectionId
                      : null;
          final sid = driverModel.value.sectionId!;
          ...
          if (activeSectionId != null &&
            ...
            await FireStoreUtils.getSectionBySectionId(activeSectionId)
                .then((sectionValue) {
              if (sectionValue != null)
                Constant.sectionModels[activeSectionId] = sectionValue;
            });
          }
          update();
        }
  ...
  Future<void> _drawGoogleRoute({
    ...
    try {
      if ((origin.latitude == 0.0 && origin.longitude == 0.0) ||
          (destination.latitude == 0.0 && destination.longitude == 0.0)) return;
  ...
  Future<BitmapDescriptor> _bitmapDescriptorFromUrl(String url,
    ...
    try {
      final Uint8List bytes =
          await Constant().getBytesFromUrl(url, width: width);
  ...
  void setOsmMapMarker() {
    ...
    final hasSource = currentOrder.value.sourceLocation != null &&
        !(currentOrder.value.sourceLocation?.latitude == null ||
            currentOrder.value.sourceLocation?.longitude == null) &&
    ...
    final hasDest = currentOrder.value.destinationLocation != null &&
        !(currentOrder.value.destinationLocation?.latitude == null ||
            currentOrder.value.destinationLocation?.longitude == null) &&
