...
class OwnerHomeController extends GetxController {
  ...
  void _processCabOrders(String driverId, List<CabOrderModel> orders) {
    final completedOrders =
        orders.where((o) => o.status == Constant.orderCompleted).toList();
    ...
    for (var order in completedOrders) {
      ...
      double driverEarned =
          (subTotal - discount + tip + taxTotal).clamp(0, double.infinity);
  ...
  double _calculateTax(
      List<TaxModel>? taxList, double subTotal, double discount) {
  ...
  int get totalCabRides => driverRideCounts.entries
      .where((e) => e.key.startsWith("cab_"))
      .fold(0, (sum, e) => sum + e.value);

  int get totalParcelRides => driverRideCounts.entries
      .where((e) => e.key.startsWith("parcel_"))
      .fold(0, (sum, e) => sum + e.value);

  int get totalRentalRides => driverRideCounts.entries
      .where((e) => e.key.startsWith("rental_"))
      .fold(0, (sum, e) => sum + e.value);

  double get totalCabEarnings => driverEarnings.entries
      .where((e) => e.key.startsWith("cab_"))
      .fold(0.0, (sum, e) => sum + e.value);

  double get totalParcelEarnings => driverEarnings.entries
      .where((e) => e.key.startsWith("parcel_"))
      .fold(0.0, (sum, e) => sum + e.value);

  double get totalRentalEarnings => driverEarnings.entries
      .where((e) => e.key.startsWith("rental_"))
      .fold(0.0, (sum, e) => sum + e.value);

  /// Grand totals across all drivers
  int get totalRidesAllDrivers => driverList.fold(
      0, (total, driver) => total + (driverRideCounts[driver.id] ?? 0));

  double get totalEarningsAllDrivers => driverList.fold(
