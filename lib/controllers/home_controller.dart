...
class HomeController extends GetxController {
  ...
  Future<void> getCurrentOrder() async {
    ...
    if (currentId != null &&
        !(driver.orderRequestData?.id == currentId) &&
    ...
    if (Constant.singleOrderReceive == true) {
      ...
      if (inProgress != null && inProgress.isNotEmpty) {
        _listenToOrder(inProgress);
        return;
      }

      if (requests != null) {
        _listenToOrder(requests.id!, checkInRequestData: true);
        return;
      }
    }
    ...
    FireStoreUtils.fireStore
        ...
        .listen((event) async {
          ...
          if (checkInRequestData &&
              !(driverModel.value.orderRequestData == newOrder.id)) {
            await _handleOrderNotFound();
            return;
          }
          ...
          if (newOrder.status == Constant.driverPending &&
              (driverModel.value.inProgressOrderID == newOrder.id)) {
            return;
          }
