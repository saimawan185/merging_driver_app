...
class RentalBookingSearchController extends GetxController {
  ...
  Future<List<RentalOrderModel>> searchParcelsOnce({
    ...
    final filtered = docs.where((doc) {
      ...
      // ✅ Check zone match
      if (data['zoneId'] == null ||
          data['zoneId'] != driverModel.value.zoneId) {
        return false;
      }
      ...
      if (orderVehicleId != null && orderVehicleId.isNotEmpty) {
        if (driverModel.value.vehicleDetails != null &&
            orderSectionId != null) {
          final sectionVehicle =
              driverModel.value.vehicleDetails![orderSectionId];
          if (sectionVehicle != null) {
            if (sectionVehicle['vehicleId']?.toString() != orderVehicleId)
              return false;
      ...
      bool isToday = orderDate.year == now.year &&
