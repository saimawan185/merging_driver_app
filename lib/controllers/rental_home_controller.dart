...
class RentalHomeController extends GetxController {
  ...
  Future<void> completeParcel(RentalOrderModel parcelBookingData) async {
    ...
    SendNotification.sendFcmMessage(Constant.rentalCompleted,
        parcelBookingData.author!.fcmToken.toString(), payLoad);
    FireStoreUtils.getRentalFirstOrderOrNOt(parcelBookingData)
        .then((value) async {
      if (value == true) {
  ...
  Future<void> updateCabWalletAmount(RentalOrderModel orderModel) async {
    ...
    if (orderModel.endTime != null) {
      ...
      if (hours >=
        ...
        hours = hours -
            int.parse(orderModel.rentalPackageModel!.includedHours.toString());
        double hourlyRate = double.tryParse(
                orderModel.rentalPackageModel?.extraMinuteFare?.toString() ??
                    "0") ??
            0.0;
    ...
    if (orderModel.startKitoMetersReading != null &&
      ...
      if (endKm > startKm) {
        ...
        if (totalKm >
          ...
          totalKm = totalKm -
              double.parse(orderModel.rentalPackageModel!.includedDistance!);
          double extraKmRate = double.tryParse(
                  orderModel.rentalPackageModel?.extraKmFare?.toString() ??
                      "0") ??
              0.0;
    ...
    if (orderModel.paymentMethod.toString() != PaymentGateway.cod.name) {
      WalletTransactionModel transactionModel = WalletTransactionModel(
          ...
          userId: orderModel.driver!.ownerId != null &&
    ...
    await FireStoreUtils.setWalletTransaction(adminCommissionTrancation)
        .then((value) async {
      if (value == true) {
        await FireStoreUtils.updateUserWallet(
