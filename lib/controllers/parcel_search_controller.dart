...
import 'package:door_delights_driver/models/parcel_category.dart';
...
class ParcelSearchController extends GetxController {
  ...
  void onInit() {
    ...
    if (driverModel.value.ownerId != null &&
        driverModel.value.ownerId!.isNotEmpty) {
  ...
  Future<void> acceptParcelBooking(ParcelOrderModel parcelBookingData) async {
    try {
      ...
      parcelBookingData.receiverPickupDateTime =
        ...
        try {
          ...
          if (fcmToken != null && fcmToken.isNotEmpty) {
            ...
            await SendNotification.sendFcmMessage(
                Constant.parcelAccepted, fcmToken, payLoad);
          }
        } catch (e) {
    ...
    for (var element in parcelBookingData.taxModel!) {
      taxAmount = (double.parse(taxAmount) +
              Constant.calculateTax(
                  ...
                  taxModel: element))
          .toStringAsFixed(
              int.tryParse(Constant.currencyModel!.decimalDigits.toString()) ??
                  2);
    }
  ...
  Future<List<ParcelOrderModel>> searchParcelsOnce({
    ...
    final driverSectionIds = driverModel.value.sectionId;
    final ref = FireStoreUtils.fireStore
        .collection("parcel_orders")
        .where("sectionId", isEqualTo: driverSectionIds)
        .where('status', isEqualTo: "Order Placed");

    GeoFirePoint center =
        Geoflutterfire().point(latitude: srcLat, longitude: srcLng);
    ...
    final filtered = docs.where((doc) {
      ...
      final zoneMatch =
          (senderZoneId == driverZoneId) || (receiverZoneId == driverZoneId);
      ...
      bool sameDay = orderDate.year == inputDate.year &&
          orderDate.month == inputDate.month &&
          orderDate.day == inputDate.day;
    ...
    try {
      return parcelCategory.firstWhere(
        (cat) =>
            ...
            parcelOrder.parcelType?.toLowerCase().trim(),
